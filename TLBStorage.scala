...
class TLBFA(
  ...
  for (i <- 0 until ports) {
    ...
    val mptperm = entries.map(_.mptperm.get)//hasmptcheck
    if (nWays == 1) {
      for (d <- 0 until nDups) {
        ...
        resp.bits.mptperm.get(d) := mptperm(0)//Hasmptcheck
      }
    } else {
      for (d <- 0 until nDups) {
        ...
        resp.bits.mptperm.get(d) :=Mux1H(hitVecReg zip mptperm)
      }
    }
...
class TLBFakeFA(
  ...
  for (i <- 0 until ports) {
    ...
    for (d <- 0 until nDups) {
      ...
      //resp.bits.mptperm.get(d):=  since it is fake,there is no need to assign an actual value to mpt perm
...
class TlbStorageWrapper(ports: Int, q: TLBParameters, nDups: Int = 1)(implicit p: Parameters) extends TlbModule {
  ...
  for (i <- 0 until ports) {
    ...
    for (d <- 0 until nDups) {
      ...
      rp.bits.mptperm.get(d) := p.bits.mptperm.get(d)//HasMptCheck
    }
  }
