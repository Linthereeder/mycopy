  ...
  with HasPerfEvents
{
  ...
  val req_in_s2xlate = (0 until Width).map(i => MuxCase(noS2xlate, Seq(//csr.mmpt.mode is implicit, when mmpt mode bare, mpt XWR 111 level 4
  ...
  val BareMode = Option.when(HasMptCheck) (!(Sv39Enable || Sv48Enable || Sv39vsEnable || Sv48vsEnable || Sv39x4Enable || Sv48x4Enable)) //bare mode, 3gates delay, dont really need to concider latency, since the value barely changes
  val portTranslateEnable = (0 until Width).map(i => (vmEnable(i) || s2xlateEnable(i)) && RegEnable(!req(i).bits.no_translate, req(i).valid))
  val mptCheckOnly = Option.when(HasMptCheck) ((0 until Width).map(i => !(vmEnable(i) || s2xlateEnable(i)) && RegEnable(!req(i).bits.no_translate, req(i).valid)))//need mptcheck even when ptw disable, but dont need it when req.no_trans 
  (0 until Width).foreach{i =>
    io.ptw.req(i).bits.mptOnly.get:=  mptCheckOnly.get(i)//assign mptCheckonly to io req
  }
  ...
  val refill = ptw.resp.fire && !(ptw.resp.bits.getGpa) && !need_gpa && !need_gpa_wire && !flush_mmu || (if(HasMptCheck) (ptw.resp.fire && BareMode.get)else false.B ) //mpt only mode also returns and save to l1tlb
  ...
  (0 until Width).foreach{i =>
    ...
    pmp_check(addr, req_out(i).size, req_out(i).cmd, noTranslateReg, i)//dont know what's doing here, where is pmp output?
    for (d <- 0 until nRespDups) {
      pbmt_check(i, d, pbmt(i)(d), g_pbmt(i)(d), req_out_s2xlate(i))
      perm_check( mptPerm.get(i)(d), perm(i)(d), req_out(i).cmd, i, d, g_perm(i)(d), req_out(i).hlvx, req_out_s2xlate(i), prepf(i), pregpf(i), preaf(i))//dont know why it is here.
    }
  ...
  def TLBRead(i: Int) = {
    val (e_hit, e_ppn, e_perm, e_g_perm, e_s2xlate, e_pbmt, e_g_pbmt,e_mptperm) = entries.io.r_resp_apply(i)//HasMptCheck,add mptperm
    val (p_hit, p_ppn, p_pbmt, p_perm, p_gvpn, p_g_pbmt, p_g_perm, p_s2xlate, p_s1_level, p_s1_isLeaf, p_s1_isFakePte,p_mptPerm) = ptw_resp_bypass(get_pn(req_in(i).bits.vaddr), req_in_s2xlate(i))//HasMptCheck,add mptperm
    val enable = portTranslateEnable(i)
    val mptonlyenable = mptCheckOnly.get(i)//when in check only mode, assign miss to true to start query mpt in l2tlb 
    ...
    val hit = e_hit || p_hit 
    val miss = (!hit && enable) || hasGpf(i) && !p_hit && !(resp_gpa_refill && need_gpa_vpn_hit) && !isOnlys2xlate && !isPrefetch && !lastCycleRedirect || (if(HasMptCheck) (mptonlyenable && !hit)  else false.B) //for mpt only either hit or miss 
    ...
    resp(i).bits.fastMiss := !hit && enable //|| (if(HasMptCheck) mptonlyenable else false.B) //dont know what fast miss is. but I assume it is safe to set it to true when in mpt only mode, since l1tlb cache will not be used  
    ...
    for (d <- 0 until nRespDups) {
      ...
      mptPerm(d):= Mux(p_hit, p_mptPerm.get, e_mptperm.get(d))//HasMptCheck
  ...
  def perm_check(mptPerm:MptPermBundle, perm: TlbPermBundle, cmd: UInt, idx: Int, nDups: Int, g_perm: TlbPermBundle, hlvx: Bool, s2xlate: UInt, prepf: Bool = false.B, pregpf: Bool = false.B, preaf: Bool = false.B) = {
    ...
    when (RegNext(prepf || pregpf || preaf)) {
      ...
      resp(idx).bits.excp(nDups).af.ld    := (af  && TlbCmd.isRead(cmd) && fault_valid) || (if(HasMptCheck) (mptRf.get || ( mptaf.get && TlbCmd.isRead(cmd)) ) else false.B )//Mpt always on and valid, even if s1 s2 in bare mode 
      resp(idx).bits.excp(nDups).af.st    := (af  && TlbCmd.isWrite(cmd) && fault_valid)|| (if(HasMptCheck) (mptWf.get || ( mptaf.get && TlbCmd.isWrite(cmd)) ) else false.B )
      resp(idx).bits.excp(nDups).af.instr := (af  && TlbCmd.isExec(cmd) && fault_valid) || (if(HasMptCheck) (mptInstf.get || ( mptaf.get && TlbCmd.isExec(cmd)) ) else false.B )
        //mptaf is af
  ...
  def handle_block(idx: Int): Unit = {
    ...
    val hit_mptOnly  = io.ptw.resp.bits.s1.hit(miss_req_vpn, Mux(hasS2xlate, csr.vsatp.asid, csr.satp.asid), csr.hgatp.vmid, allType = true, false, hasS2xlate)//mpt only mode use s1 as return tag
    val hit = (if(HasMptCheck) mptCheckOnly.get(idx) && hit_mptOnly && io.ptw.resp.valid else false.B )|| Mux(onlyS2, hit_s2, hit_s1) && io.ptw.resp.valid && miss_req_s2xlate_reg === io.ptw.resp.bits.s2xlate
    ...
    resp(idx).valid := req_out_v(idx) && (!(miss_v && portTranslateEnable(idx)) || (if(HasMptCheck)!(miss_v && mptCheckOnly.get(idx)) else false.B))//dont know what this line is used for.
    when (io.ptw.resp.fire && hit && req_out_v(idx) && (portTranslateEnable(idx)|| (if(HasMptCheck)  mptCheckOnly.get(idx) else false.B) )) {//add condition, eqvelently make it =true.B, always return 
      ...
      for (d <- 0 until nRespDups) {
        ...
        perm_check(mptresp, stage1, req_out(idx).cmd, idx, d, stage2, req_out(idx).hlvx, s2xlate)
      }
