...
class PTW()(implicit p: Parameters) extends XSModule with HasPtwConst with HasPerfEvents {
  ...
  val enableS2xlate = req_s2xlate =/= noS2xlate//假设 enable = false， 会发生什么？
  val onlyS1xlate = req_s2xlate === onlyStage1//假设 enable = false， 会发生什么？
  val onlyS2xlate = req_s2xlate === onlyStage2//假设 enable = false， 会发生什么？
  ...
  val flush = io.sfence.valid || io.csr.satp.changed || io.csr.vsatp.changed || io.csr.hgatp.changed|| (if(HasMptCheck) (io.csr.mmpt.changed) else false.B)//changes in mpt flush all
  ...
  val pte = if (HasBitmapCheck) { Mux(jmp_bitmap_check_r, cache_pte.get, io.mem.resp.bits.asTypeOf(new PteBundle().cloneType)) } 
    else if(HasMptCheck) Mux(io.mem.resp.valid, io.mem.resp.bits.asTypeOf(new PteBundle().cloneType), RegEnable(io.mem.resp.bits.asTypeOf(new PteBundle().cloneType),io.mem.resp.valid))
    // mem.resp is only valid for limited time,use reg to extend its life, for mpt may takes long. timing + 3gates 
  ...
  val mpt_af = if(HasMptCheck) {RegEnable(!io.mptCheck.get.resp.bits.mptPerm(0) ||io.mptCheck.get.resp.bits.accessFault , io.mptCheck.get.resp.valid)} else false.B
  ...
  ptw_resp.apply(Mux(pte_valid, pageFault && !(accessFault||(if(HasMptCheck)(mpt_af) else false.B)), false.B), (accessFault||(if(HasMptCheck)(mpt_af) else false.B)) || (ppn_af && !(pte_valid && (pageFault || guestFault))), Mux((accessFault||(if(HasMptCheck)(mpt_af) else false.B)), af_level, Mux(guestFault, gpf_level, level)), Mux(pte_valid, pte, fake_pte), vpn, satp.asid, hgatp.vmid, vpn(sectortlbwidth - 1, 0), not_super = false, not_merge = false, bitmap_checkfailed.asBool)

  val normal_resp = idle === false.B && mem_addr_update && !need_last_s2xlate && (guestFault || (w_mem_resp && find_pte) || (s_pmp_check && accessFault) || onlyS2xlate ||(if(HasMptCheck)(mpt_af) else false.B) )
  ...
  io.llptw.valid := s_llptw_req === false.B && to_find_pte && !(accessFault||(if(HasMptCheck)(mpt_af) else false.B)) && !guestFault
  io.llptw.bits.req_info.source := source
  io.llptw.bits.req_info.vpn := vpn
  io.llptw.bits.req_info.s2xlate := req_s2xlate
  if(HasMptCheck){io.llptw.bits.req_info.mptOnly.get := false.B}
  ...
  if(HasMptCheck){//向mpt发送req
    io.mptCheck.get.req.valid:= s_mpt_check
    io.mptCheck.get.req.bits.mptOnly:=false.B//mpt during tw is never mpt only 
    io.mptCheck.get.req.bits.reqPA:= Mux(s2xlate, hpaddr(PAddrBits-1 ,offLen), mem_addr(PAddrBits-1 ,offLen))
    io.mptCheck.get.req.bits.id:= DontCare//id assigned in l2tlb
  }
  ...
  mem.req.valid := s_mem_req === false.B && !mem.mask && !accessFault && s_pmp_check &&(if(HasMptCheck)(!mpt_af) else true.B)
  ...
  if(HasMptCheck){io.refill.req_info.mptOnly.get := false.B}
  ...
  when(sent_to_pmp && mem_addr_update === false.B){//send pmp 
    if(HasMptCheck) {when(mptEn) {s_mpt_check:= true.B} .otherwise {s_mem_req := false.B} } else {s_mem_req := false.B}//send to mpt /send to mem,  when pmp pass)
    s_pmp_check := true.B//checked
  }
  //add mpt  
  if(HasMptCheck) {
  ...
  when((accessFault || (if (HasMptCheck) (mpt_af) else false.B )) && !io.hptw.req.valid && idle === false.B){
    s_pmp_check := true.B
    s_mem_req := true.B
    w_mem_resp := true.B
    s_llptw_req := true.B
  ...
  when(mem_addr_update){
    when(level >= 2.U && !onlyS2xlate && !(guestFault || find_pte || accessFault||(if(HasMptCheck) mpt_af else (false.B)))) {
      level := levelNext
      when(s2xlate){
        s_hptw_req := false.B
      }.otherwise{
        if(HasMptCheck) { s_mpt_check:= true.B}//mem_addr_update checks pmp, next is mpt or mem access state
        s_mem_req := false.B
      }
      s_llptw_req := true.B
      ...
      need_last_s2xlate := false.B
      when(!(guestFault || accessFault || pageFault || ppn_af||(if(HasMptCheck) mpt_af else (false.B)))){//no fault 
        s_last_hptw_req := false.B
        mem_addr_update := false.B
      }
...
class LLPTW(implicit p: Parameters) extends XSModule with HasPtwConst with HasPerfEvents {
  ...
  val flush = io.sfence.valid || io.csr.satp.changed || io.csr.vsatp.changed || io.csr.hgatp.changed || (if(HasMptCheck)( io.csr.mmpt.changed ) else false.B)
  ...
  val mpt_arb = Option.when(HasMptCheck) ( Module(new RRArbiterInit(new LLPTWEntry(), l2tlbParams.llptwsize)))//switch mpt input,arb
  if(HasMptCheck){    
  ...
  val dup_vec_mpt = Option.when(HasMptCheck)(dup_vec.zipWithIndex.map{case(d, i) => d && (is_stage_mpt_req(i) || is_stage_mpt_resp(i))}) //input is dup of mpt entry
  ...
  when (pmp_resp_valid) {
    ...
    state(ptr) := Mux(accessFault, state_mem_out, (if(HasMptCheck) Mux(mptEn, stage_mpt_req, state_mem_req) else state_mem_req))
  ...
  if(HasMptCheck){
    when (mpt_arb.get.io.out.fire){
      for (i <- state.indices) {
        ...
        && dup(entries(i).req_info.vpn, mpt_arb.get.io.out.bits.req_info.vpn)) {//不懂这儿的state =/=是否是必要的，dup重复但是处于state idle？ 可能出现吗
          ...
          //entries(i).hptw_resp := entries(mpt_arb.get.io.chosen).hptw_resp 没用
          entries(i).wait_id := mpt_arb.get.io.chosen//entries 记录内部arb id 用于返回时比较
        }
      }
    }
  
    when (io.mptCheck.get.resp.valid) {
      val accessFault = !(io.mptCheck.get.resp.bits.mptPerm(0)) || io.mptCheck.get.resp.bits.accessFault // for ptw, af is af or not readable 
      state.indices.map{i =>
        when (state(i) === stage_mpt_resp && io.mptCheck.get.resp.bits.id === entries(i).wait_id) {//对所有相同的wait id 进行检查
          entries(i).af:=accessFault// 如果af为1，那么不会走到这个state，因此无需accessFault||state.af
          state(i) := Mux(accessFault, state_mem_out, state_mem_req)//af just return
        }
      }
    }
  }
  ...
  if(HasMptCheck){//mpt output assign send to mptarb
    ...
    io.mptCheck.get.req.bits.reqPA:=  Mux(mpt_arb.get.io.out.bits.req_info.s2xlate === allStage, mpt_hpaddr, mpt_paddr)
    io.mptCheck.get.req.bits.id:= DontCare//assign by mpt arb in l2tlb
...
class HPTW()(implicit p: Parameters) extends XSModule with HasPtwConst {
  ...
  val sfence = io.sfence
  val flush = sfence.valid || hgatp.changed || io.csr.satp.changed || io.csr.vsatp.changed|| (if(HasMptCheck)(io.csr.mmpt.changed ) else false.B)
  val mode = hgatp.mode
...
//  val pte = io.mem.resp.bits.MergeRespToPte()
  ...
  val pte = if (HasBitmapCheck) Mux(jmp_bitmap_check, cache_pte.get, io.mem.resp.bits.asTypeOf(new PteBundle().cloneType)) 
    else if(HasMptCheck) Mux(io.mem.resp.valid, io.mem.resp.bits.asTypeOf(new PteBundle().cloneType), RegEnable(io.mem.resp.bits.asTypeOf(new PteBundle().cloneType),io.mem.resp.valid))
    // mem.resp is only valid for limited time,use reg to extend its life, for mpt may takes long. timing + 3gates 
    else io.mem.resp.bits.asTypeOf(new PteBundle().cloneType)
  ...
  val mpt_af = if(HasMptCheck) {RegEnable(!io.mptCheck.get.resp.bits.mptPerm(0) || io.mptCheck.get.resp.bits.accessFault, io.mptCheck.get.resp.valid)} else false.B//R=0, af or not readable=>af , mpt af will be reged
  ...
  resp.apply(
    ...
    gaf = accessFault || (ppn_af && !pageFault)|| (if(HasMptCheck) mpt_af else false.B),
    level = Mux(accessFault|| (if(HasMptCheck) mpt_af else false.B), af_level, level),
    pte = pte, 
    vpn = vpn, 
    vmid = hgatp.vmid
  )
  ...
  if(HasMptCheck){//向mpt发送req
    io.mptCheck.get.req.valid:= s_mpt_check
    io.mptCheck.get.req.bits.mptOnly:=false.B//mpt during tw is never mpt only 
    io.mptCheck.get.req.bits.reqPA:= mem_addr(PAddrBits-1 ,offLen)//和PMP一致，但是不用offset
    io.mptCheck.get.req.bits.id:= DontCare //id 由arbiter赋予
  }
  ...
  when (idle){
    ...
    when (io.req.fire && (if (HasBitmapCheck) !io.req.bits.bitmapCheck.get.jmp_bitmap_check else true.B)) {
      ...
      gpaddr := Cat(io.req.bits.gvpn, 0.U(offLen.W))
      accessFault := false.B
      if(HasMptCheck) {mpt_af := false.B}//reset mptaf
      s_pmp_check := false.B
      id := io.req.bits.id
  ...
  when(sent_to_pmp && !mem_addr_update){
    if(HasMptCheck) {when(mptEn) {s_mpt_check:= true.B} .otherwise {s_mem_req := false.B} } //to mptcheck state，send valid
    else {s_mem_req := false.B}//send to mpt /send to mem,  when pmp pass)
    s_pmp_check := true.B
  }

  when(!idle && (accessFault || (if (HasMptCheck) (mpt_af) else false.B )) ){//pmp return af
    s_pmp_check := true.B
    s_mem_req := true.B
    w_mem_resp := true.B
    mem_addr_update := true.B
  ...
  when(mem_addr_update){//下一级
    when(!(find_pte || accessFault||(if(HasMptCheck) mpt_af else (false.B)))){// 结束条件 not, 继续下一级
      level := levelNext
      if(HasMptCheck) { s_mpt_check:= true.B}//mem_addr_update checks pmp, next is mpt or mem access state
      else {s_mem_req := false.B}
      mem_addr_update := false.B// 不再update mem addr
    }.elsewhen(resp_valid){
      when(io.resp.fire){
        idle := true.B
        mem_addr_update := false.B
