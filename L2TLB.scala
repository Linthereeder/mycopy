...
class L2TLBImp(outer: L2TLB)(implicit p: Parameters) extends PtwModule(outer) with HasCSRConst with HasPerfEvents {
  ...
  val mptOnlyPort = 5 // req directly from l1tlb. in this situation, adress translation is disabled and the reqVPN is the actuall PA waiting to be mpt checked. 
  val lastMptPort2 = 4
  ...
  val ptwMptPort = 0
  if(HasMptCheck) {
    mptc.get.io.req <> mpt_arb.get.io.out
  }
  ...
  arb1.io.out.ready := (arb2.io.in(InArbTlbPort).ready && tlbCounter < MissQueueSize.U) && (if(HasMptCheck) !(mpt_arb.get.io.in(mptOnlyPort).ready) else true.B) //arb1 out is ready when arb2 and mptc are both ready
  ...
  if (HasMptCheck) {
    ...
    llptw.io.mptCheck.get.resp.valid:= mptc.get.io.resp.valid && mptc.get.io.resp.bits.id === llptwMptPort.U
  }
  ...
  if (HasMptCheck) {
    ...
    ptw.io.mptCheck.get.resp.valid:= mptc.get.io.resp.valid && mptc.get.io.resp.bits.id === ptwMptPort.U
  }
  ...
  if (HasMptCheck) {
    ...
    hptw.io.mptCheck.get.resp.valid:= mptc.get.io.resp.valid && mptc.get.io.resp.bits.id === hptwMptPort.U
  }
  ...
  def from_mptc (id: UInt) = {// HasMptCheck
    id === l2tlbParams.llptwsize.U + 2.U
  }
  ...
  mptc.get.io.mem.mask := waiting_resp.apply(l2tlbParams.llptwsize + 2)
  ...
  val resp_pte = VecInit((0 until (if (HasBitmapCheck) MemReqWidth / 2 else MemReqWidth)).map(i =>//mpt en memreq+1
    if (i == l2tlbParams.llptwsize + 2) {RegEnable(get_part(refill_data_tmp, req_addr_low(i)), 0.U.asTypeOf(get_part(refill_data_tmp, req_addr_low(i))), mem_resp_done && mem_resp_from_mptc) } // HasMptChecker
  ...
  if(HasMptCheck){
    mptc.get.io.mem.resp.valid := mem_resp_done && mem_resp_from_mptc
  ...
  if(HasMptCheck){
    for (i <- 0 until PtwWidth) {
      val gpa = mergeArb(i).out.bits.s1.genPPN() //use tag 
      val hpa = mergeArb(i).out.bits.s2.genPPNS2(mergeArb(i).out.bits.s2.entry.tag) //tag or gpa? for better timing, use tag 
      val mptreqPa = Mux(mergeArb(i).out.bits.s2xlate === onlyStage1, gpa,hpa)
      //val outarb_needmptcheck:= mergeArb(i).out.valid
    ...
    //llptw.io.mptCheck.req.ready:= mpt_arb.get.io.in(llptwMptPort).ready
      ...
      when(outArb(i).out.fire){//fire resets mpt result reg
        outarb_mptcheckDone.get(i):= false.B
      }
    }
  }
  ...
  for (i <- 0 until PtwWidth) {
    ...
    if(HasMptCheck){ 
      ...
      when(outarb_mptres.get(i).mptonly ){
        ...
        outArb(i).in(0).bits.s1.entry.tag:= outarb_mptres.get(i).reqPA
        outArb(i).in(0).bits.s1.addr_low := outarb_mptres.get(i).reqPA(sectortlbwidth-1,0) 
        outArb(i).in(0).bits.s1.entry.level.get := 3.U
        for (i <- 0 until PtwWidth) {
          outArb(i).in(0).bits.s1.valididx(i) := true.B
          outArb(i).in(0).bits.s1.ppn_low(i) := i.U
        }
      }
    }  
  }
